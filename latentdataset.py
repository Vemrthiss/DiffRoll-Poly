...
class LatentAudioDataset(Dataset):
  def __init__(self, h5_dir, audio_dir, chunk_duration_sec=27):
    ...
    self.h5_files = glob.glob(os.path.join(h5_dir, '**', '*.hdf5'), recursive=True)
    self.h5_dir = h5_dir
    self.audio_dir = audio_dir
    self.chunk_duration_sec = chunk_duration_sec # this was done in the latent extract phase, by default 27
  ...
  def __getitem__(self, idx):
    ...
    piano_roll, _ = tsv2roll(chunk_tsv, audio_length, sr, hop_length, max_midi=108, min_midi=21)
...
def collate_fn(batch):
  ...
  for roll in piano_roll_list:
    ...
    piano_roll_mask = torch.cat([torch.ones(length), torch.zeros(pad_length)])
