...
class RollDiffusion(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        batch_size = batch["piano_roll"].shape[0]
        batch = batch["piano_roll"].unsqueeze(1)
        device = batch.device
...
class SpecRollDiffusion(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        roll_label = batch["piano_roll"].unsqueeze(1).cpu()
...
#                       [127]*len(p_est))
    ...
    def step(self, batch):
        ...
        # batch["audio"] (B, L)
        if isinstance(batch, list):
            batch_size = batch[0]["piano_roll"].shape[0]
            roll = self.normalize(
                batch[0]["piano_roll"]).unsqueeze(1)  # (B, 1, T, F)
            latents = batch[0]["dac_latents"]  # (B, 72, 2325) for example
        else:
            batch_size = batch["piano_roll"].shape[0]
            roll = self.normalize(
                batch["piano_roll"]).unsqueeze(1)  # (B, 1, T, F)
            latents = batch["dac_latents"]  # (B, 72, 2325) for example
        ...
        if self.hparams.training.mode == 'epsilon':
            if self.hparams.debug == True:
                ...
                # predict the noise N(0, 1)
                epsilon_pred, spec = self(x_t, latents, t)
            ...
            # predict the noise N(0, 1)
            pred_roll, spec = self(x_t, latents, t)
            diffusion_loss = self.p_losses(
            ...
            if isinstance(batch, list):  # when using multiple dataset do one more feedforward
                ...
                pred_roll2, spec2 = self(
                    x_t2, latents2, t, sampling=True)  # sampling = True
    ...
    def ddpm(self, x, latents, t_index):
        ...
        # Equation 11 in the paper
        # Use our model (noise predictor) to predict the mean
        epsilon, spec = self(x, latents, t_tensor)
    ...
    def ddpm_x0(self, x, latents, t_index):
        ...
        # Equation 11 in the paper
        # Use our model (noise predictor) to predict the mean
        x0_pred, spec = self(x, latents, t_tensor)
...
#             return (model_mean + torch.sqrt(posterior_variance_t) * noise), spec
    ...
    def cfdg_ddpm_x0(self, x, latents, t_index):
        ...
        # Equation 11 in the paper
        # Use our model (noise predictor) to predict the mean
        x0_pred_c, spec = self(x, latents, t_tensor)
...
class LatentRollDiffusion(pl.LightningModule):  # Renamed from SpecRollDiffusion
    def __init__(self,
                 lr,
                 timesteps,
                 loss_type,
    ...
    def test_step(self, batch, batch_idx):
        ...
        # to see if it is working there, because my tensors should be of same shapes as theirs
        # I am binarizing the roll_label, BUT THIS MIGHT BE WRONG?
        frame_p, frame_r, frame_f1, _ = precision_recall_fscore_support(roll_label.flatten() > self.hparams.frame_threshold,
                                                                        roll_pred.flatten() > self.hparams.frame_threshold,
                                                                        average='binary')
